...
for i = 1:num_bins
    ...
    C0 = movmean(abs(wprcoef(T,[3 2])./std(wprcoef(T,[3 2]))),movmeanw);
    C1 = movmean(abs(wprcoef(T,[4 6])./std(wprcoef(T,[4 6]))),movmeanw);
    C2 = movmean(abs(wprcoef(T,[4 7])./std(wprcoef(T,[4 7]))),movmeanw);
    C3 = movmean(abs(wprcoef(T,[3 4])./std(wprcoef(T,[3 4]))),movmeanw);
    C4 = movmean(abs(wprcoef(T,[3 5])./std(wprcoef(T,[3 5]))),movmeanw);
    C5 = movmean(abs(wprcoef(T,[3 6])./std(wprcoef(T,[3 6]))),movmeanw);
    C6 = movmean(abs(wprcoef(T,[3 7])./std(wprcoef(T,[3 7]))),movmeanw);
...
signal = sum(p_norm(:,b_interest(2:3)),2);

[pks, locs] = findpeaks(signal,'MinPeakProminence', 0.5, 'MinPeakDistance',0);
...
%% == Plot FFT of signal ==
% This step is not necessary for the analysis
signal = fft(p_filtered(:,4)); % signal to do FFT on
Fs = 100;
L = num_frames;
